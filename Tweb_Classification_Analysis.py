...
Instructions:
    ...
    4. Make sure to change the smoothing scales in the input_params.txt file as per the requirement before running this script.
       The code will perform calculations for the smoothing scales specified in the input_params.txt file.
...
def get_box_size() -> int:
    ...
    try:
        ...
        raise logging.error("Box size not found in the 'simulation_properties.txt' file.")
    
    except FileNotFoundError as e:
        logging.error(f"'simulation_properties.txt' file does not exist in current working directory: {e}")
        raise
...
def load_density_fields() -> list[np.ndarray]:
    ...
    density_fields = load_all_npy_files(folder_path=os.path.join(save_path, 'smoothed_density_fields'), 
                                        filename_prefix='smoothed_density_field_', str_smoothing_scales=truncated_scales)
...
def load_classification_matrices() -> list[np.ndarray]:
    """
    ...
    classifications = load_all_npy_files(folder_path=classification_path, 
                                         filename_prefix='classification_matrix_', str_smoothing_scales=truncated_scales)
...
def plot_volfrac_vs_rs(classifications: list[np.ndarray], smooth_scales: list[float], dir_path: str):
    """
...
def plot_structure_classification(classifications: np.ndarray, density_fields: np.ndarray, smooth_scales: list[float], 
    ...
    for i in range(len(smooth_scales)):
        plot_classification_overlay(smth_scale = smooth_scales[i], 
                                    ...
                                    save_path = os.path.join(dir_path, f'Classification_overlay_{truncated_scales[i]}_{projection}.png'))

#------------------------------ PLOT CLASSIFICATION OVERLAY ON DENSITY FIELD --------------------------------#

def plot_all_environments_overlay(classifications: np.ndarray, density_fields: np.ndarray, smooth_scales: list[float], 
                                  truncated_scales: list[str], slice_thickness: list[int, int], projection: str, slice_index: int, dir_path: str):
    """
    ...
    for i in range(len(smooth_scales)):
        overlay_all_envs(classification_matrix=classifications[i], smoothed_rho=density_fields[i], 
                        slice_thickness=slice_thickness, 
                        smoothing_scale=smooth_scales[i],
                        slice_index=slice_index,
                        projection=projection,
                        grid_size=grid_size, box_size=box_size,
                        save_path=os.path.join(dir_path, f'All_Environments_Overlay_{truncated_scales[i]}_{projection}.png'))
...
def plot_structure_changes(transformations: dict[str, np.ndarray], density_field: np.ndarray, truncated_scales: list[str],  dir_path: str, 
                           slice_thickness: list[int, int], projection: str, unique_num: int) -> None:
    """
    ...
    plot_env_changes(transformations = transformations, 
                    density_slice = slice_density_field(density_field, slice_thickness=slice_thickness,projection=projection),
                    box_size=box_size, 
...
if __name__ == '__main__':
    try:
        ...
        projection = 'xy' # Specify the projection to be used. Options: 'xy', 'yz', 'xz'
        ...
        # Plot the classification overlay on density field for all smoothing scales
        plot_structure_classification(classifications, smoothed_density_fields, smooth_scales, truncated_scales, 
                                      new_directory_path, slice_thickness, slice_index, projection)

        plot_all_environments_overlay(classifications, smoothed_density_fields, smooth_scales, truncated_scales, 
                                    slice_thickness, projection, slice_index, new_directory_path)
        ...
        if len(smooth_scales) > 1:
            ...
            # Get all environment changes
            env_changes = get_structure_changes(classifications, slice_index, projection)

            # Generate the transformation dictionary
            transformations_1, transformations_2 = generate_transformation_dictionary(env_changes)

            # Plot the changes in classification of Halo environments for above transformations
            plot_structure_changes(transformations_1, 
                                smoothed_density_fields[len(smoothed_density_fields)//2], 
                                truncated_scales, new_directory_path, slice_thickness, projection, unique_num=1)
            ...
            logging.info('Skipping the volume fraction plot & changes in structure classification plots....\n')
